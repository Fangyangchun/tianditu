G.Geocoding = {
	...
	initMap : function() {
		var that = this;
		var options = {
			center: [30.26870728, 120.14888763],
			zoom: 12,
			zoomControl: false
		}
		...
		//矢量瓦片
		var vLayer = new L.GXYZ('https://ditu.zjzwfw.gov.cn/mapserver/vmap/zjvmap/getMAP?x={x}&y={y}&l={z}&styleId=tdt_biaozhunyangshi_2017',{tileSize:512,maxZoom: 20,minZoom:3});
		that.map.addLayer(vLayer);
		//注记图层
		var vLabel = new L.GWVTAnno({tileSize:512});
		...
		vLabel.addDataSource(dataSource);
		that.map.addLayer(vLabel);
		this.vLabel = vLabel;
	...
	addGasj:function(){
	    ...
	    dataSource1.url = 'https://ditu.zjzwfw.gov.cn/mapserver/label/gasj/getDatas?x=${x}&y=${y}&l=${z}&styleId=gasj_gasj_20171012';
	    dataSource1.id = "label_gasj";
	    this.vLabel.addDataSource(dataSource1);
	    this.vLabel.redraw();
	},
	initEvent : function() {
		var that = this;
		//公安数据叠加
		$('.coad-ope .change').click(function(){
			$(this).toggleClass("on");
			if($(this).hasClass('on')){
				that.vLabel.removeDataSourceById("label_gasj");
				that.vLabel.redraw();
			}else{
				that.addGasj();
			}
		...
		//绑定键盘事件
		$(".sg-input").keydown(function(e) {
            if (e.keyCode == 13) {
                $('.search-tab-form .src-tab-list .src-btn').click();
            }
        });
		$(".jd-input, .wd-input").keydown(function(e) {
            if (e.keyCode == 13) {
                $('.search-tab-form .src-tab-list .src-btn').click();
            }
        });
